...
FROM
    TEMP
GROUP BY id_voto , dpi , mesa , fecha_hora;
...
SELECT 
    Partido.nombre AS partido,
    SUM(CASE
        WHEN C.cargo = 3 THEN 1
        ELSE 0
    END) AS 'Diputados congreso lista nacional',
    SUM(CASE
        WHEN C.cargo = 4 THEN 1
        ELSE 0
    END) AS 'Diputados congreso distrito electoral',
    SUM(CASE
        WHEN C.cargo = 5 THEN 1
        ELSE 0
    END) AS 'Diputados parlamento centroamericano',
...
WHERE
    C.cargo IN (3 , 4, 5)
GROUP BY Partido.id , Partido.nombre;

-- Alcande por partido
SELECT 
    Partido.nombre AS partido, C.nombre AS alcalde
FROM
    Candidato C
        INNER JOIN
...
SELECT 
    P.nombre AS partido,
    SUM(CASE
        WHEN C.cargo = 1 THEN 1
        ELSE 0
    END) AS presidente,
    SUM(CASE
        WHEN C.cargo = 2 THEN 1
        ELSE 0
    END) AS vicepresidente,
    SUM(CASE
        WHEN
            C.cargo = 3 OR C.cargo = 4
                OR C.cargo = 5
        THEN
            1
        ELSE 0
...
SELECT 
    D.nombre AS DEPARTAMENTO, COUNT(V.id) AS 'CANTIDAD DE VOTOS'
FROM
    DEPARTAMENTO D
        INNER JOIN
    MESA M ON M.departamento = D.id
        INNER JOIN
    VOTO V ON V.mesa = M.id
GROUP BY D.nombre , D.id
;

-- Votos nulos
SELECT 
    COUNT(DISTINCT DV.voto) AS 'VOTO NULOS'
FROM
    DETALLE_VOTO DV
        INNER JOIN
    VOTO V ON V.id = DV.voto
WHERE
    DV.candidato = - 1;

-- Top edades
SELECT 
    C.edad, COUNT(C.dpi) AS Cantidad
FROM
    VOTO V
        INNER JOIN
...
SELECT 
    V.mesa, D.nombre, COUNT(V.mesa) AS 'Cantidad votantes'
FROM
    VOTO V
        INNER JOIN
...
SELECT 
    TIME(V.fecha_hora) AS HORA, COUNT(V.fecha_hora) AS CANTIDAD
FROM
    VOTO V
GROUP BY V.fecha_hora
ORDER BY COUNT(V.fecha_hora) DESC
LIMIT 5;

-- Votos por genero
SELECT 
    C.genero, COUNT(C.genero) AS 'Cantidad de votos'
FROM
    CIUDADANO C
        INNER JOIN
